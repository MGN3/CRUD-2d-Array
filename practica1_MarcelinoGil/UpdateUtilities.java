...
public class UpdateUtilities {
	 ...
	 * <li>In case one of the fields contains data, the column wont be consedered to
	 * be empty, and then the update is availeable for the user.
	 * </ul>
	 * 
	...
	public static void updateRow(String[][] arrayString) {
		...
		if (!emptyRow) {
			...
			System.out.println("The selected row is empty. A row must have data in order to modify it.");
			System.out.println('\n');
		}
		...
		for (int row = 0; row < rowsLength; row++) {
			...
			for (int col = 0; col < columnsLength; col++) {
				if (arrayString[row][col] != null && !rowDiscarded && arrayString[row][col].equals(dataRequest)) {
					...
					System.out.print((row + 1) + "  "); //prints autoincremental id
					for (int i = 0; i < columnsLength; i++) {
						System.out.print("- " + arrayString[row][i] + " -"); //Prints every field
					}
